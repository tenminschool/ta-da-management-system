...
export function bandPolicy(
  policy: Policy,
  band: string,
): BandPolicy | undefined {
  return policy.bands.find(
    (b) => b.band.toUpperCase() === String(band || '').toUpperCase(),
  );
}
...
export function daySpan(
  fromDate: string,
  toDate: string,
): { total: number; weekday: number; weekend: number } {
 ...
 */
export function personalVehicleRateFor(
  policy: Policy,
  user: SessionUser,
): number | null {
...
export function bankPayoutAllowed(policy: Policy): boolean {
  return (
    String(cfgStr(policy, 'ALLOW_BANK_PAYOUT', 'No'))
      .trim()
      .toLowerCase() === 'yes'
  );
}
 ...
 */
export function femaleTravelling(
  gender: string,
  travelType: string,
  teamGenders: string[] = [],
): boolean {
...
export function eligibleModes(
  ...
  const push = (mode: string, enabled: boolean, reason = '') => {
    ...
    out.push({
      ...
      enabled,
      reason,
      requiresReceipt: s.requiresReceipt,
    });
  };
  ...
  if (ctx.scope === 'inside') {
    ...
    const female =
      femaleTravelling(ctx.gender, ctx.travelType, ctx.teamGenders) ||
      !!ctx.exceptionClaimed;
    const allowed =
      (isFemaleGender(ctx.gender) ? band?.modesFemale : band?.modesMale) ?? [];
    ...
    for (const mode of ['Rickshaw', 'Bike', 'CNG', 'Car']) {
        ...
        if (female) {
          ...
          if (ctx.teamSize >= teamMin) push('Car', true);
            ...
            push(
              'Car',
              false,
              `Car needs at least ${teamMin} travellers — currently ${ctx.teamSize}.`,
            );
        } else if (inBand) {
          push('Car', true);
          ...
          push(
            'Car',
            false,
            'Your band needs pre-approval for Car. Tick “Car pre-approved” to claim it.',
          );
        }
        continue;
      }
      ...
      if (mode === 'Rickshaw' && ctx.teamSize > 2) {
        push(
          'Rickshaw',
          false,
          `Rickshaw seats two — there are ${ctx.teamSize} of you.`,
        );
        continue;
      }
  ...
  for (const s of policy.modes) {
    ...
    if (s.mode === 'Flight') {
      if (band?.flightEligible) push('Flight', true);
        ...
        push('Flight', false, `Flight is not available for Band ${ctx.band}.`);
      continue;
    }
    if (s.mode === 'RentACar') {
      if (ctx.travelType !== 'team') {
        push(
          'RentACar',
          false,
          'Rent-a-car needs a team pooling together — not available for individual travel.',
        );
      } else if (
        band?.carPoolEligible ||
        femaleTravelling(ctx.gender, ctx.travelType, ctx.teamGenders) ||
        ctx.exceptionClaimed
      ) {
        push('RentACar', true);
        ...
        push(
          'RentACar',
          false,
          `Rent-a-car pooling is not available for Band ${ctx.band}.`,
        );
      }
      continue;
    }
 ...
 */
export function insidePerTravellerAmount(
  policy: Policy,
  hours: number,
...
export function computeRequest(
  ...
  if (!band)
    errors.push(
      `No policy is configured for Band "${user.band}". Contact Administration.`,
    );
  ...
  if (draft.scope === 'inside' && user.insideCityBlocked) {
    errors.push(
      "You can't apply for inside-city travel. Contact Administration if this needs to change.",
    );
  }
  ...
  const companyArranged =
    draft.scope === 'outside' && draft.arrangement === 'company';
  ...
  let fuelRate = 0;
  const legTotal = companyArranged
    ? 0
    : draft.legs.reduce((sum, l) => {
  ...
  if (draft.scope === 'inside' && legTotal > 0) {
    notes.push(
      'Inside-city transport is reimbursed against actual receipts where one is issued.',
    );
  } else if (draft.scope === 'outside' && legTotal > 0) {
    notes.push(
      'Intercity travel is reimbursed at actual against tickets/receipts.',
    );
  }
  ...
  if (draft.legs.some((l) => l.mode === 'PersonalVehicle')) {
    ...
    if (!user.registeredVehicle) {
      errors.push(
        'Register your vehicle and get it approved by HR or Admin before claiming a personal-vehicle trip.',
      );
    } else {
      const rate = personalVehicleRateFor(policy, user);
      /* eslint-disable-next-line @typescript-eslint/no-unused-vars -- fuelRate is never read after this; pre-existing dead store inherited from the pre-migration codebase, left as-is rather than changed during the port. */
  ...
  if (draft.scope === 'outside') {
    ...
    const perHead = money(
      span.weekday * (band?.outsideTAWeekday ?? 0) +
        span.weekend * (band?.outsideTAWeekend ?? 0),
    );
    ...
    if (perDiemEligible) {
      notes.push(
        `Outside-city Per-Diem for Band ${user.band}: ${span.weekday} weekday × ${band?.outsideTAWeekday ?? 0} + ${span.weekend} weekend × ${band?.outsideTAWeekend ?? 0} = ${perHead} each` +
          (teamSize > 1
            ? `, × ${teamSize} travellers = ${perDiemAmount}`
            : '') +
    ...
    const travellerMeals = [
      officeMeal,
      ...draft.teamMembers.map((m) => m.officeMealTaken),
    ];
    if (hours >= minHours) {
      ...
      const mealDeduction = cfgNum(policy, 'OFFICE_MEAL_DEDUCTION', 75);
      perDiemAmount = money(
        travellerMeals.reduce(
          (sum, mealTaken) =>
            sum + insidePerTravellerAmount(policy, hours, mealTaken),
          0,
        ),
      );
      ...
      notes.push(
        `Worked ${hours} hours (≥ ${minHours}) — Per-Diem ${perHead} each` +
          (anyMeal
            ? `, minus ${mealDeduction} for whoever had the office meal`
            : '') +
          (teamSize > 1
            ? ` — ${teamSize} travellers, ${perDiemAmount} total`
            : '') +
      ...
      const shortHourAllowance = cfgNum(
        policy,
        'SHORT_HOUR_MEAL_ALLOWANCE',
        150,
      );
      const withoutMeal = travellerMeals.filter(
        (mealTaken) => !mealTaken,
      ).length;
  ...
  if (draft.dualWorkstation) {
    notes.push(
      `Dual workstation (${draft.dualWorkstationType || 'unspecified'}) — TA and Per-Diem are allowed, company meal is assumed, duplicate meal claims are blocked.`,
    );
    if (!draft.dualWorkstationType)
      errors.push('Select a dual-workstation reason.');
  }
  ...
  let accommodationAmount =
    draft.accommodationType === 'personal' || companyArranged
      ? 0
  ...
  if (accommodationAmount > 0) {
    if (draft.scope !== 'outside') {
      ...
      if (!draft.hotelName)
        errors.push('Hotel name is required for an accommodation claim.');
      if (!draft.checkIn || !draft.checkOut)
        errors.push(
          'Check-in and check-out dates are required for an accommodation claim.',
        );
      if (accommodationAmount > accommodationLimit && accommodationLimit > 0) {
        ...
        notes.push(
          `Accommodation within the Band ${user.band} limit (${perNightLimit}/night × ${nights || 1} = ${accommodationLimit}).`,
        );
      }
    }
  }
  ...
  const rentACarAmount = money(
    rentACarLegs.reduce((s, l) => s + (Number(l.amount) || 0), 0),
  );
  if (rentACarLegs.length) {
    ...
    const femaleParty = femaleTravelling(
      user.gender,
      draft.travelType,
      draft.teamMembers.map((m) => m.gender),
    );
    if (draft.travelType !== 'team') {
      ...
      errors.push(
        `Rent-a-car needs at least ${minHead} employees — ${head} entered. Request rejected by policy.`,
      );
    } else if (rentACarLegs.some((l) => Number(l.amount) > limit)) {
      errors.push(
        `Rent-a-car cannot exceed ${limit} one-way — check each rent-a-car leg.`,
      );
    } else {
      notes.push(
        `Rent-a-car pooled across ${head} employees, within the ${limit} one-way limit.`,
      );
    }
  }
  ...
  const flightAmount = money(
    flightLegs.reduce(
      (s, l) => s + (Number(l.amount) || 0) * (teamSize > 1 ? teamSize : 1),
      0,
    ),
  );
  ...
  if (otherAmount > 0 && !draft.otherNote)
    warnings.push('Explain the “other” amount so Finance can verify it.');
  ...
  const advanceAvailable =
    draft.scope === 'outside' && tripDays >= advanceMinDays && advanceNoticeOK;
  let advanceRequested =
    draft.advanceWanted && draft.advanceType === 'full'
      ? totalClaim
  ...
  if (advanceRequested > 0) {
    ...
    notes.push(
      `Settlement is due within ${cfgNum(policy, 'ADVANCE_SETTLEMENT_DAYS', 3)} working days of the trip ending.`,
    );
  }
  ...
  const claimType: ClaimType =
    paidTA && paidPerDiem ? 'both' : paidPerDiem ? 'perdiem' : 'ta';
  if (draft.scope === 'inside' && (paidTA || paidPerDiem)) {
    notes.push(
      `Claim resolved automatically as ${
        { ta: 'TA only', perdiem: 'Per-Diem only', both: 'TA + Per-Diem' }[
          claimType
        ]
  ...
  const endedOn =
    draft.scope === 'outside' ? draft.toDate || draft.fromDate : draft.fromDate;
  ...
  if (draft.scope === 'inside') {
    ...
    const chosen = policy.destinationTypes.find(
      (d) =>
        ...
        (!d.cities.length || d.cities.includes(draft.city)),
    );
    ...
    if (cityZone(policy, draft.city) === 'Outside')
      errors.push(`${draft.city} is not an inside-city location.`);
    if (!draft.startTime || !draft.endTime)
      errors.push('Start and end time are required to calculate Per-Diem.');
    ...
    if (!draft.toDate)
      errors.push('Return date is required for outside-city travel.');
    if (tripDays <= 0)
      errors.push('The return date cannot be before the travel date.');
    ...
    if (!taken) errors.push('Select a route.');
      ...
      errors.push(`Which city did you travel to from ${taken.from}?`);
    else if (taken.to && draft.city !== taken.to)
      errors.push(`${taken.label} must end in ${taken.to}.`);
    // Dhaka has several offices, so whichever end of the route is Dhaka needs
    // to say which one — Chattogram, so far, does not.
    if (
      taken &&
      (taken.from === 'Dhaka' || taken.to === 'Dhaka') &&
      !draft.dhakaOffice
    ) {
    ...
    if (draft.arrangement === 'company') {
      if (!companyArrangementOK) {
        ...
        notes.push(
          `Company arrangement requested with ${noticeGiven} business days' notice — Administration will be notified automatically.`,
        );
      }
    }
  }
  ...
  if (!companyArranged) {
    ...
    else if (draft.legs.some((l) => !l.mode))
      errors.push('Select a travel mode for every trip.');
    if (
      draft.legs.some(
        (l) => l.mode === 'PersonalVehicle' && !(Number(l.amount) > 0),
      )
    ) {
  ...
  if (draft.travelType === 'team') {
    if (draft.teamMembers.length < 1)
      errors.push(
        'Add at least one team member, or switch back to Individual travel.',
      );
    else {
      ...
      const pooledTransport = draft.legs.some(
        (l) =>
          l.mode &&
          l.mode !== 'CompanyVehicle' &&
          !PER_TRAVELLER_MODES.has(l.mode) &&
          Number(l.amount) > 0,
      );
  ...
  const payoutMethod =
    draft.payoutMethod === 'bank' && bankAllowed ? 'bank' : 'bkash';
    ...
    if (payoutMethod === 'bank') {
      ...
      for (const [label, value] of bankFields) {
        if (!String(value || '').trim())
          errors.push(`${label} is required for a bank payment.`);
      }
    } else {
      const bkashOK = (label: string, v: string) => {
        const bkash = String(v || '').replace(/[\s-]/g, '');
        if (!bkash)
          errors.push(`Enter the bKash number ${label} should be paid to.`);
        else if (!/^01[3-9]\d{8}$/.test(bkash)) {
          errors.push(
            `${label}'s bKash number does not look right — it should be 11 digits starting 01, e.g. 01712345678.`,
          );
        }
      };
      ...
      if (draft.travelType === 'team') {
        draft.teamMembers.forEach((m) =>
          bkashOK(m.name || 'your teammate', m.bkashNumber),
        );
        ...
        const entries = [
          {
            label: 'You',
            number: String(draft.bkashNumber || '').replace(/[\s-]/g, ''),
          },
        ...
        for (const e of entries) {
          ...
          if (clash) {
            errors.push(
              `${clash} and ${e.label} can't share the same bKash number — each traveller's number must be unique.`,
            );
          } else {
  ...
  if (malformed.length) {
    errors.push(
      `These are not valid links: ${malformed.slice(0, 3).join(', ')}. Paste the full URL starting with https://`,
    );
  }
  ...
  const anyLegNeedsReceipt = draft.legs.some(
    (l) => policy.modes.find((m) => m.mode === l.mode)?.requiresReceipt,
  );
  const needsReceipt =
    anyLegNeedsReceipt || accommodationAmount > 0 || otherAmount > 0;
  if (
    cfgStr(policy, 'REQUIRE_DOCUMENT_LINK', 'Yes').toLowerCase() === 'yes' &&
    needsReceipt &&
    !links.length
  ) {
    errors.push(
      'Share at least one document link (Drive, bill, ticket or receipt) supporting this claim.',
    );
  }
  if (links.some((l) => /drive\.google\.com|docs\.google\.com/i.test(l))) {
    notes.push(
      `${links.length} document link(s) attached — approvers open them directly, so keep the Drive sharing open to them.`,
    );
  }
 ...
 */
export function teamPayoutSplit(
  record: RequestRecord,
  policy: Policy,
): TeamPayout[] {
  const teamSize =
    record.travelType === 'team' ? record.teamMembers.length + 1 : 1;
  if (teamSize <= 1) {
    return [
      {
  ...
  const insidePerDiemEligible =
    record.scope === 'inside' && record.perDiemDays > 0;
  const ownPerDiemAndLunch = (mealTaken: boolean) =>
    record.scope !== 'inside'
  ...
  const requesterOnlyShared = money(
    record.accommodationAmount + record.otherAmount,
  );
  const requesterMealTaken = record.dualWorkstation
    ? true
    : record.officeMealTaken;
  const requesterOwn = money(
    perHeadTa + requesterSharedTa + ownPerDiemAndLunch(requesterMealTaken),
  );
  const memberOwn = (m: (typeof record.teamMembers)[number]) =>
    money(perHeadTa + perHeadSharedTa + ownPerDiemAndLunch(m.officeMealTaken));

  const claimTotal = money(
    requesterOwn +
      record.teamMembers.reduce((s, m) => s + memberOwn(m), 0) +
      requesterOnlyShared,
  );
  const payableTotal =
    record.approvedAmount > 0 ? record.approvedAmount : record.totalClaim;
  const scale = claimTotal > 0 ? payableTotal / claimTotal : 1;

  const requesterAmount = money(
    (requesterOwn + requesterOnlyShared) * scale - record.advanceRequested,
  );
  ...
  return [
    ...
    ...record.teamMembers.map((m) => ({
      ...
      name: m.name,
      bkashNumber: m.bkashNumber,
      amount: money(memberOwn(m) * scale),
    })),
  ];
}
...
export function impliedLegs(policy: Policy, draft: RequestDraft): Leg[] {
  ...
  const leg = (
    ...
    travelTo: string,
    mode = '',
  ): Leg => ({ travelDate, mode, travelFrom, travelTo, amount: 0, note: '' });

  if (draft.scope === 'inside') {
    const kind = policy.destinationTypes.find(
    ...
    if (kind?.needs === 'office') {
      ...
      if (draft.tripDirection === 'two_way')
        return [outward, leg(draft.fromDate, office, 'HQ')];
      return [outward];
    }
    ...
    if (draft.tripDirection === 'two_way')
      return [outward, leg(draft.fromDate, there, draft.city)];
    return [outward];
  }
...
export function emptyDraft(
  scope: Scope = 'inside',
): RequestDraft & { carSpecialApproval: boolean } {
