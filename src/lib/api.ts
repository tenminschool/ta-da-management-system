/**
 * Typed fetch wrapper for ta-da's own backend (app/api/**), not the
 * template's apiUrl()/API_SERVICES pattern — that's for calling 10MS's
 * genuinely external REST API with the raw 10MS OAuth token; this app
 * exchanges that token once at login for an app-specific session token
 * (see /api/auth/tenms) and uses that as Bearer on every same-origin call.
 */
...
export const api = {
  uploadConfig: () =>
    call<{ enabled: boolean; maxBytes: number }>('/uploads/config'),
  ...
  upload: async (
    ...
    const { uploadUrl } = await post<{ uploadUrl: string; name: string }>(
      '/uploads/session',
      {
        name: file.name,
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            ...
            reject(
              new Error(
                'Drive accepted the file but returned an unexpected response.',
              ),
            );
          }
        } else {
    ...
    const { file: saved } = await post<{
      file: { id: string; name: string; link: string; sizeBytes: number };
    }>('/uploads/finish', { fileId: uploaded.id });
    return saved;
  },
  ...
  saveBkashNumber: (bkashNumber: string) =>
    post<{ ok: boolean; bkashNumber: string }>('/me/bkash', { bkashNumber }),
  saveTeammateBkash: (employeeId: string, bkashNumber: string) =>
    post<{ ok: boolean; employeeId: string; bkashNumber: string }>(
      `/employees/${encodeURIComponent(employeeId)}/bkash`,
      { bkashNumber },
    ),
  policy: () => call<Policy>('/policy'),
  insideCityBlock: () =>
    call<{ entries: InsideCityBlockEntry[] }>('/admin/inside-city-block'),
  addInsideCityBlock: (email: string, note: string) =>
    post<{ entry: InsideCityBlockEntry }>('/admin/inside-city-block', {
      email,
      note,
    }),
  updateInsideCityBlock: (currentEmail: string, email: string, note: string) =>
    call<{ entry: InsideCityBlockEntry }>(
      `/admin/inside-city-block/${encodeURIComponent(currentEmail)}`,
      {
        method: 'PUT',
  ...
  removeInsideCityBlock: (email: string) =>
    call<{ ok: boolean }>(
      `/admin/inside-city-block/${encodeURIComponent(email)}`,
      { method: 'DELETE' },
    ),
  employees: (q: string) =>
    call<{ employees: EmployeeLite[] }>(
      `/employees?q=${encodeURIComponent(q)}`,
    ),
  ...
  request: (id: string) =>
    call<RequestDetail>(`/requests/${encodeURIComponent(id)}`),
  preview: (draft: RequestDraft) =>
    post<{ computation: Computation; modes: ModeOption[] }>(
      '/requests/preview',
      { draft },
    ),
  create: (draft: RequestDraft, submit: boolean) =>
    post<{ request: RequestRecord; computation: Computation }>('/requests', {
      draft,
      submit,
    }),
  update: (id: string, draft: RequestDraft, submit: boolean) =>
    call<{ request: RequestRecord; computation: Computation }>(
  ...
  saveCompanyAmounts: (
    ...
    post<{ request: RequestRecord }>(
      `/requests/${encodeURIComponent(id)}/company-amounts`,
      { entries },
    ),
  pay: (id: string, payload: Record<string, unknown>) =>
    post<{ request: RequestRecord }>(
      `/requests/${encodeURIComponent(id)}/payment`,
      payload,
    ),
  reconcilePreview: (contentBase64: string) =>
    post<ReconcileResult>('/requests/payment-reconcile/preview', {
      contentBase64,
    }),
  ...
  acknowledge: (id: string, received: boolean, note = '') =>
    post<{ request: RequestRecord }>(
      `/requests/${encodeURIComponent(id)}/acknowledge`,
      { received, note },
    ),

  claimUnlock: (employeeId: string, from: string) =>
    post<{ ok: boolean; employeeId: string; from: string }>(
      '/admin/claim-unlock',
      { employeeId, from },
    ),

  requestUnlock: (reason: string, requestedFrom: string) =>
    post<{ request: UnlockRequest }>('/unlock-requests', {
      reason,
      requestedFrom,
    }),
  ...
  decideUnlock: (
    ...
    post<{ request: UnlockRequest }>(
      `/unlock-requests/${encodeURIComponent(id)}/decide`,
      { action, remarks, unlockFrom },
    ),

  myVehicle: () =>
    call<{ vehicle: VehicleRegistration | null }>('/vehicles/mine'),
  ...
  decideVehicle: (
    ...
    post<{ vehicle: VehicleRegistration }>(
      `/vehicles/${encodeURIComponent(employeeId)}/decide`,
      { action, remarks },
    ),

  advances: (scope: 'mine' | 'desk') =>
    call<{ requests: (RequestRecord & { myStep: AdvanceStep | null })[] }>(
      `/advances?scope=${scope}`,
    ),
  advanceAction: (id: string, payload: Record<string, unknown>) =>
    post<{ request: RequestRecord }>(
      `/requests/${encodeURIComponent(id)}/advance`,
      payload,
    ),
  ...
  paymentExport: async (
    ...
    const filename =
      ...
      )?.[1] || 'bkash-payment.xlsx';
    const skippedHeader = res.headers.get('X-Skipped-Ids');
    const skipped = skippedHeader
      ? decodeURIComponent(skippedHeader).split(',')
      : [];
    const skippedTravellersHeader = res.headers.get('X-Skipped-Travellers');
    const skippedTravellers = skippedTravellersHeader
      ? decodeURIComponent(skippedTravellersHeader).split(',')
      : [];
