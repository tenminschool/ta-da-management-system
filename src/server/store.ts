...
export async function loadPolicy(): Promise<Policy> {
  ...
  const policy: Policy = {
    ...
    cities: of('City').map((r) => ({
      city: r.value,
      zone: (r.extra_1 === 'Outside' ? 'Outside' : 'Inside') as
        | 'Inside'
        | 'Outside',
    })),
    modes: of('TransportMode').map((r) => ({
      ...
      scope: (['Inside', 'Outside', 'Both'].includes(r.extra_1)
        ? r.extra_1
        : 'Both') as 'Inside' | 'Outside' | 'Both',
    ...
    destinationTypes: of('DestinationType').map((r) => ({
      value: r.value,
      label: r.label || r.value,
      needs: (r.extra_1 === 'Office'
        ? 'office'
        : r.extra_1 === 'Purpose'
          ? 'purpose'
          : 'name') as 'name' | 'office' | 'purpose',
    ...
    fuelTypes: of('FuelType').map((r) => ({
      value: r.value,
      label: r.label || r.value,
      pricePerLitre: num(r.extra_1),
    })),
...
export async function allEmployees(): Promise<EmployeeRow[]> {
  if (employeeCache && Date.now() - employeeCache.at < EMPLOYEE_TTL_MS)
 ...
 */
export async function rememberAuthId(
  employeeRow: string,
  authId: string,
): Promise<void> {
...
export async function deptHeadIdFor(employeeId: string): Promise<string> {
  ...
  const lineManager = employee?.lineManagerId
    ? byId.get(employee.lineManagerId)
    : undefined;
  if (!lineManager || lineManager.employeeId === employeeId) return '';
  const head = lineManager.lineManagerId
    ? byId.get(lineManager.lineManagerId)
    : undefined;
  ...
  return members
    .map((m) =>
      [
        ...
        m.gender,
        m.bkashNumber,
        m.companyTransportAmount || '',
        m.companyAccommodationAmount || '',
        yn(m.officeMealTaken),
      ].join(' | '),
    )
    .join('\n');
}
...
function packTrips(legs: Leg[]): string {
  return legs
    .map((l) =>
      [l.travelDate, l.mode, l.travelFrom, l.travelTo, l.amount, l.note].join(
        ' | ',
      ),
    )
    .join('\n');
...
export function toRequest(
  ...
  return {
    ...
    city: r.city,
    claimType: (['ta', 'perdiem', 'both'].includes(r.claim_type)
      ? r.claim_type
      : 'both') as RequestRecord['claimType'],
    ...
    companyTransportAmount: num(r.company_transport_amount),
    companyAccommodationAmount: num(r.company_accommodation_amount),
    companyAmountsBy: r.company_amounts_by || '',
    companyAmountsAt: r.company_amounts_at || '',
    ...
    accommodationType:
      r.accommodation_type === 'hotel' || r.accommodation_type === 'personal'
    ...
    advanceType:
      r.advance_type === 'full' || r.advance_type === 'partial'
        ? r.advance_type
        : '',
...
export function fromRequest(req: RequestRecord): Row {
  return {
    ...
    company_transport_amount: String(req.companyTransportAmount || ''),
    company_accommodation_amount: String(req.companyAccommodationAmount || ''),
    company_amounts_by: req.companyAmountsBy,
    company_amounts_at: req.companyAmountsAt,
    linked_to: req.linkedTo,
    ...
    accommodation_type: req.accommodationType,
...
export function toVehicle(
  r: Row & { _row: string },
): VehicleRegistration & { _row: string } {
  return {
    _row: r._row,
    employeeId: r.employee_id,
    employeeName: r.employee_name,
    ...
    status: (['pending', 'approved', 'rejected'].includes(r.status)
      ? r.status
      : 'pending') as VehicleRegistration['status'],
    submittedAt: r.submitted_at,
...
export function toUnlockRequest(
  r: Row & { _row: string },
): UnlockRequest & { _row: string } {
  return {
    _row: r._row,
    requestId: r.request_id,
    employeeId: r.employee_id,
    ...
    status: (['pending', 'approved', 'rejected'].includes(r.status)
      ? r.status
      : 'pending') as UnlockRequest['status'],
...
export function toInsideCityBlockEntry(
  r: Row & { _row: string },
): InsideCityBlockEntry & { _row: string } {
  return {
    _row: r._row,
    email: r.email,
    note: r.note,
...
export async function insideCityBlockedEmails(): Promise<Set<string>> {
  if (
    insideCityBlockCache &&
    Date.now() - insideCityBlockCache.at < INSIDE_CITY_BLOCK_TTL_MS
  )
    return insideCityBlockCache.emails;
  ...
  await withSheetLock(async () => {
    ...
    for (const patch of patches) {
      ...
      if (
        ...
        patch.group !== 'AdvanceHR' &&
        patch.group !== 'AdvanceDeptHead'
      ) {
