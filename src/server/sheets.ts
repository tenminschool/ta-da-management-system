...
class RateLimiter {
  ...
  private refill(): void {
    ...
    this.tokens = Math.min(
      this.burst,
      this.tokens + ((now - this.lastRefill) / 60_000) * this.perMinute,
    );
    this.lastRefill = now;
  }
    ...
    const run = async () => {
      for (;;) {
        ...
        const waitMs = Math.max(
          25,
          ((1 - this.tokens) / this.perMinute) * 60_000,
        );
...
const readLimiter = new RateLimiter(
  Number(process.env.SHEETS_READS_PER_MINUTE) || 150,
  10,
);
const writeLimiter = new RateLimiter(
  Number(process.env.SHEETS_WRITES_PER_MINUTE) || 150,
  10,
);
...
export async function readTab(
  ...
  values.forEach((raw, i) => {
    ...
    if (!raw || raw.every((c) => c === '' || c === null || c === undefined))
      return;
...
export async function readTabs(
  tabs: string[],
): Promise<Record<string, (Row & { _row: string })[]>> {
  const headerSets = await Promise.all(tabs.map((t) => getHeaders(t)));
  const ranges = tabs.map(
    (t, i) =>
      `${quote(t)}!A2:${colLetter(Math.max(headerSets[i].length - 1, 0))}`,
  );
  const res = await retry(() =>
