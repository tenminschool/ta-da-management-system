...
const HEADER_LABELS = ["receipt no.", "withdrawn", "opposite party", "transaction status", "details", "completion time"];
 ...
 */
export function parseSettlementSheet(worksheet: ExcelJS.Worksheet): SettlementRow[] {
  ...
  for (let r = headerRow + 1; r <= worksheet.rowCount; r++) {
    const row = worksheet.getRow(r);
    const cell = (label: string) => (colMap[label] ? row.getCell(colMap[label]).value : undefined);
 ...
 */
export function matchSettlement(fileRows: SettlementRow[], candidates: MatchCandidate[]): ReconcileResult {
  ...
  for (const [number, claimList] of claimsByNumber) {
    const fileList = (fileByNumber.get(number) ?? []).filter((r) => !usedFileRows.has(r));
    if (!fileList.length) {
  ...
  const unmatchedFileRows = fileRows.filter((r) => r.bkashNumber && !usedFileRows.has(r));
  const order: Record<Confidence, number> = { exact: 0, close: 1, mismatch: 2 };
  matches.sort((a, b) => order[a.confidence] - order[b.confidence] || a.amountDiff - b.amountDiff);
