...
const nn = (v: string | undefined) => (v === undefined || v === null ? "" : String(v));
...
async function readLive(title: string): Promise<Rec[]> {
  ...
  return values.slice(1)
    ...
    .map((row) => Object.fromEntries(headers.map((h, i) => [h, nn(row[i] as string)])));
}

async function main() {
  if (!process.env.SPREADSHEET_ID || !process.env.GOOGLE_CLIENT_EMAIL) {
    throw new Error("Missing SPREADSHEET_ID / GOOGLE_CLIENT_EMAIL — check your .env file.");
  }
  ...
  console.log(`Existing tabs (${existing.size}): ${[...existing.keys()].join(", ") || "(none)"}\n`);

  // ── 1. Snapshot anything that needs remapping, before headers change ──────
  const snapshot: Record<string, Rec[]> = {};
  for (const t of ["Requests", "Approvals", "RequestLegs", "TeamMembers", "Payments", "Advances"]) {
    if (existing.has(t)) snapshot[t] = await readLive(t);
  }
  const oldRequests = snapshot.Requests?.length && "AdvanceApprovedAmount" in snapshot.Requests[0];
  const oldApprovals = snapshot.Approvals?.length && "ApprovalID" in snapshot.Approvals[0];
  if (oldRequests || oldApprovals) {
    console.log(`Previous layout detected — migrating ${snapshot.Requests?.length || 0} request(s).\n`);
  }
  ...
  const addRequests = TABS.filter((t) => !existing.has(t.title)).map((t) => {
    ...
    return {
      addSheet: {
        properties: {
          title: t.title,
          gridProperties: { rowCount: 1000, columnCount: Math.max(t.headers.length, 8), frozenRowCount: 1 },
          tabColor: t.color,
        },
      },
    };
  ...
  if (addRequests.length) {
    const res = await sheets.spreadsheets.batchUpdate({ spreadsheetId: id, requestBody: { requests: addRequests } });
  ...
  const widen = TABS
    .filter((t) => !created.includes(t.title))
    .map((t) => ({
      updateSheetProperties: {
        properties: { sheetId: existing.get(t.title)!, gridProperties: { columnCount: Math.max(t.headers.length, 8) } },
  ...
  if (oldRequests) {
    ...
    console.log(`Migrated ${rows.length} request row(s) — trips, team, payment and advance folded in.`);
  }
  ...
  if (oldApprovals) {
    ...
    const STATE: Record<string, string> = {
      ...
      Paid: "Paid", "Documents Requested": "Documents Requested",
    };
    const byReq = groupBy(snapshot.Approvals, (r) => r.RequestID);
    const nameOf = Object.fromEntries((snapshot.Requests || []).map((r) => [r.RequestID, r.EmployeeName]));
    const headers = TABS.find((t) => t.title === "Approvals")!.headers;

    const rows = Object.entries(byReq).map(([requestId, entries]) => {
      const rec: Rec = { RequestID: requestId, EmployeeName: nn(nameOf[requestId]) };
      entries
        .slice()
      ...
      const req = (snapshot.Requests || []).find((r) => r.RequestID === requestId);
    ...
    console.log(`Collapsed the approval log into ${rows.length} row(s) — one per request.`);
  }
  ...
  const topUp: { title: string; key: (r: unknown[]) => string; label: (r: unknown[]) => string }[] = [
    // A setting is identified by its key; a list value by which list it is in.
    { title: "Config", key: (r) => String(r[0] ?? "").trim(), label: (r) => String(r[0]) },
    { title: "Lists", key: (r) => `${String(r[0] ?? "").trim()} ${String(r[1] ?? "").trim()}`, label: (r) => String(r[0]) },
  ];
  for (const { title, key, label } of topUp) {
    ...
    console.log(`  ${title}: added ${missing.length} missing entr(y/ies) — ${[...new Set(missing.map(label))].join(", ")}`);
  }
  ...
  for (const t of TABS) {
    ...
    fmt.push({
      updateSheetProperties: {
        properties: { sheetId, gridProperties: { frozenRowCount: 1 }, tabColor: t.color },
    ...
    fmt.push({
      repeatCell: {
        range: { sheetId, startRowIndex: 1, startColumnIndex: 0, endColumnIndex: t.headers.length },
    ...
    fmt.push({
      setBasicFilter: {
        filter: { range: { sheetId, startRowIndex: 0, startColumnIndex: 0, endColumnIndex: t.headers.length } },
      },
    });
  }
  ...
  if (removable.length) {
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId: id,
      requestBody: { requests: removable.map((t) => ({ deleteSheet: { sheetId: existing.get(t)! } })) },
    });
